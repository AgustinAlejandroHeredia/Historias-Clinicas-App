...
import { agregarHistoriaClinica, eliminarHistoriaClinica, obtenerHistoriaClinicaCompletaPorId } from "@/db/historia_clinica_service";
import { agregarLineaTiempoItem, eliminarItemPorId, obtenerItemsPorHistoriaId } from "@/db/linea_tiempo_item_service";
import { agregarPariente, eliminarParientePorId, obtenerParientesPorHistoria } from "@/db/pariente_service";
...
export default function Edit() {
    ...
    useEffect(() => {
        const obtenerDatos = async () => {
            try {
                ...
                console.log("idit : Datos cargados exitosamente ✅")
            }catch (error) {
                console.error("idit : Error al obtener la informacion a presentar ❌: ", error)
            }
        }
            obtenerDatos()
    }, []);
    ...
    const obtenerDatosHistoria = async () => {
        ...
        if(!response.success) {
            throw new Error("idit : Error obteniendo historia clinica ❌ ")
        }
    ...
    const obtenerDatosItems = async () => {
        ...
        if(!response.success) {
            throw new Error("idit : Error obteniendo items ❌ ")
        }
    ...
    const obtenerDatosParientes = async () => {
        ...
        if (!response.success) {
            throw new Error("idit: Error obteniendo parientes ❌")
        }
